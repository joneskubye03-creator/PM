...
export default function Dashboard() {
  ...
  // Tight follow-camera flag (Bolt/Uber-style) passed to the map.
  ...
  // Decide when the map should tightly follow the live vehicle. Pro apps briefly
  // show the whole route after a phase change (handled by the map's FIT_BOUNDS
  // on a new polyline), then snap into a close head-up follow view.
  useEffect(() => {
    if (!isOnline) {
    ...
    if (showPolyline) {
      // A new route leg was just drawn: let the FIT_BOUNDS overview play, then
      // switch into the close follow camera.
      setFollowVehicle(false);
      const t = setTimeout(() => setFollowVehicle(true), 4000);
      return () => clearTimeout(t);
    }
    // Online with no active route -> follow the driver closely.
    setFollowVehicle(true);
  }, [isOnline, showPolyline, activePolyline, tripStatus]);
